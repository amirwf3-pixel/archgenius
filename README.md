...
| شاخص | مقدار تأییدشده |
|---|---|
| آزمون‌های خودکار | ۸۵۴ تست PASS در ۴۶ فایل (هسته) + ۶۴ تست PASS (وب) |
| Type Check هسته (Core) | PASS — TypeScript strict |
| Type Check رابط کاربری (Web) | PASS |
| بیلد رابط کاربری | PASS — ۳۲۱ ماژول (Vite) |
...
- **یافته‌های SOFT برخورد آرکِ در — برطرف شده در Phase 18**: اعتبارسنجی قبلی یک هیوریستیک جعبه‌محیطی بود که هر لولای نزدیکِ T-جانکشن را نشانه می‌رفت؛ اکنون هر دو یافته (`OPENING_DOOR_SWING_BLOCKED` / `FURNITURE_BLOCKS_DOOR`) با هندسه دقیق قطاع ۹۰ درجهٔ خودِ مدل Opening بررسی می‌شوند و چیدمان مبلمان نیز از قطاع چرخش دور می‌ماند. در سناریوهای نماینده، شمار این دو یافته صفر است.
...
- **۸۵۴ تست PASS** در **۴۶ فایل تست هسته** + **۶۴ تست PASS وب** — بدون fail و بدون skip.
- **Core TSC**: PASS (TypeScript strict).
- **Web TSC**: PASS.
- **بیلد وب**: PASS — **۳۲۱ ماژول** (Vite).

...
describe('Phase 7 — XLSX', () => {
  it('workbook generation with expected sheets', async () => {
    const prj = createProject(baseInput());
    const { bestCandidate } = generate(prj);
    const doc = buildDocumentation(prj, bestCandidate);
    ...
    expect(res.ok).toBe(true);
    expect(res.sheets).toEqual(['01_Project', '02_Room_Schedule', '03_Area_Summary', '04_Openings', '05_QA', '06_Regulations']);
...
describe('Phase 7 — Manifest', () => {
  it('serialization and versioning', async () => {
    const prj = createProject(baseInput());
    const { bestCandidate } = generate(prj);
    const doc = buildDocumentation(prj, bestCandidate);

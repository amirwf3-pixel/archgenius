...
export interface QAReport {
  ...
  intelligence?: DocumentationModel['intelligence'];
  wholeBuilding?: {
    floorCount: number;
    perFloor: { floorLevel: number; overallQuality: number; intelligenceScope: string }[];
    vertical: { score: number; isConnected: boolean; disconnectedFloors: number[]; stairCount: number; isHeuristic: boolean };
    stacking: { score: number; kitchenStackingScore: number; bathroomStackingScore: number; isHeuristic: boolean };
    interFloor: { score: number; entranceToVerticalScore: number; floorAccessScore: number; isHeuristic: boolean };
    wholeBuilding: { overall: number; avgFloorQuality: { overall: number }; intelligenceScope: string; evaluableWeightsSum: number };
  };
  site?: DocumentationModel['site'];
  metadata: {
    version: string;
    generatedAt: string;
...
export function buildQAReport(docModel: DocumentationModel): QAReport {
  ...
  // Phase 9 whole-building summary for report
  const intel = docModel.intelligence;
  const wholeBuilding = intel ? {
    floorCount: intel.floorCount,
    perFloor: intel.perFloor.map(pf => ({ floorLevel: pf.floorLevel, overallQuality: pf.overallQuality, intelligenceScope: pf.intelligenceScope })),
    vertical: { score: intel.vertical.score, isConnected: intel.vertical.isConnected, disconnectedFloors: intel.vertical.disconnectedFloors, stairCount: intel.vertical.stairCount, isHeuristic: intel.vertical.isHeuristic },
    stacking: { score: intel.stacking.score, kitchenStackingScore: intel.stacking.kitchenStackingScore, bathroomStackingScore: intel.stacking.bathroomStackingScore, isHeuristic: intel.stacking.isHeuristic },
    interFloor: { score: intel.interFloor.score, entranceToVerticalScore: intel.interFloor.entranceToVerticalScore, floorAccessScore: intel.interFloor.floorAccessScore, isHeuristic: intel.interFloor.isHeuristic },
    wholeBuilding: { overall: intel.wholeBuilding.overall, avgFloorQuality: { overall: intel.wholeBuilding.avgFloorQuality.overall }, intelligenceScope: intel.wholeBuilding.intelligenceScope, evaluableWeightsSum: (intel.wholeBuilding as any).evaluableWeightsSum ?? 1 },
  } : undefined;

...
/** Compute buildable footprint from site + packs. */
export function computeBuildableArea(project: ProjectInput): BuildableFootprint {
  ...
  // Apply street-side reduction: on access side, smaller setback is typical.
    ...
    appliedRules: [
      {
        ...
        reference: 'Default assumption pack',
        message: `Applied assumed setbacks N=${setbacks.north} S=${setbacks.south} E=${setbacks.east} W=${setbacks.west} m.`,
        status: 'REQUIRES_SOURCE_VERIFICATION',
      },
    ],
